...
export const UniverseTable = (props) => {
  const {
          ...
          {isAvailable(currentCustomer.data.features, 'costs.universe_list') ? (
            <div className="cell-cost">
              <div className="cell-cost-value">
                <YBCost value={universe.pricePerHour} multiplier="month" />
              </div>
              /month
            </div>
          ) : (
...
const CellResourcesPanel = (props) => {
  const {universe: { universeUUID, readData, writeData }} = props;
