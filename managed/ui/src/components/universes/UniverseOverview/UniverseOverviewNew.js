...
const HealthHeart = ({ status }) => {
  return (
    <div id="health-heart">
      <span className={`fa fa-heart${status === 'loading' ? ' status-loading' : ''}`} />
      {status === 'success' && (
...
HealthHeart.propTypes = {
  status: PropTypes.string
};

const AlertInfoPanel = ({ alerts, universeInfo }) => {
    ...
    const healthCheckInfoItems = [
      {
        ...
        data: errorNodesCounter ? (
          ...
          <span className="text-green text-light">
            <i className={'fa fa-check'} /> All running fine
          </span>
        )
      },
      {
        ...
        data: lastUpdateDate ? (
          <span className="text-lightgray text-light">
            <i className={'fa fa-clock-o'} /> Updated{' '}
...
AlertInfoPanel.propTypes = {
  alerts: PropTypes.object,
  universeInfo: PropTypes.object
};

const HealthInfoPanel = ({ healthCheck, universeInfo }) => {
...
HealthInfoPanel.propTypes = {
  healthCheck: PropTypes.object.isRequired,
  universeInfo: PropTypes.object.isRequired
};

export const UniverseOverviewNew = (
  {
    universe: {
      currentUniverse: {
        data: { universeUUID }
    ...
    updateAvailable, currentCustomer,
  ...
  const getLastUpdateDate = () => {
    const universeTasks = tasksForUniverse();
  ...
  const getCostWidget = (currentUniverse) => {
  ...
  const getPrimaryClusterWidget = (currentUniverse) => {
  ...
  const getTablesWidget = (universeInfo) => {
    if (isNullOrEmpty(tables)) return;
  ...
  const getHealthWidget = (healthCheck, universeInfo) => {
    return (
  ...
  const getAlertWidget = (alerts, universeInfo) => {
    return (
      <Col lg={4} md={8} sm={8} xs={12}>
  ...
  const getDiskUsageWidget = (universeInfo) => {
  ...
  const getCPUWidget = (universeInfo) => {
  ...
  const getRegionMapWidget = (universeInfo) => {
    const isItKubernetesUniverse = isKubernetesUniverse(universeInfo);
    const mapWidget = (
      <YBWidget
        numNode
  ...
  const getDatabaseWidget = (universeInfo, tasks) => {
    const lastUpdateDate = getLastUpdateDate();
    const showUpdate = updateAvailable && !isDisabled(currentCustomer.data.features, 'universes.actions');
            ...
            <FlexShrink>
              {lastUpdateDate && (
                <div className="text-lightgray text-light">
                  <span className={'fa fa-clock-o'} /> Upgraded{' '}
                  <span className={'text-dark text-normal'}>
                    <FormattedDate
    ...
    <Fragment>
      <Row>
        ...
        {isDisabled(currentCustomer.data.features, 'universes.details.health')
          ...
          : getHealthWidget(universe.healthCheck, universeInfo)}
      </Row>
      <Row>
