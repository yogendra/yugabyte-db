...
export const UserProfileForm = (
  {
    ...
    changeUserPassword,
    refreshApiToken,
    customerProfile, handleProfileUpdate
  }
) => {
  const [statusUpdated, setStatusUpdated] = useState(false);
  const [prevCustomerProfile, setPrevCustomerProfile] = useState(customerProfile);

  const handleRefreshApiToken = (e) => {
  ...
  useEffect(() => {
    const hasProfileChanged =
      getPromiseState(prevCustomerProfile) !== getPromiseState(customerProfile) &&
      (getPromiseState(customerProfile).isSuccess() ||
        getPromiseState(customerProfile).isError());
    if (statusUpdated && hasProfileChanged) {
      handleProfileUpdate(customerProfile.data);
      setStatusUpdated(false);
      setPrevCustomerProfile(customerProfile)
    }
  }, [customerProfile]);
