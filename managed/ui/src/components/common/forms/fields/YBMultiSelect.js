...
export const YBNewMultiSelect = ({ input, options, isMulti, isReadOnly, selectValChanged,providerSelected, ...otherProps }) => {
  const [prevIsMulti, setPrevIsMulti] = useState(isMulti);
  const [prevInput, setPrevInput] = useState(input);
  const [prevProviderSelected, setPrevProviderSelected] = useState(providerSelected);

  useComponentDidUpdate(() => {
    let newSelection = null;

    // If AZ is changed from multi to single, take only last selection.
    if (isMulti !== prevIsMulti && isMulti === false) {
      const currentSelection = prevInput.value;
    ...
    if (providerSelected !== prevProviderSelected) {
      setPrevProviderSelected(providerSelected)
      newSelection = [];
    }

    if (isNonEmptyArray(newSelection) && isFunction(input.onChange)) {
      input.onChange(newSelection);
    }
    setPrevIsMulti(isMulti);
    setPrevInput(input);
  }, [isMulti, input, providerSelected]);
...
export const YBMultiSelectWithLabel = ({ label, meta, ...otherProps } ) => {
  return (
