...
export const RestoreBackup = (
  {
    onHide, onSubmit, onError, restoreTableBackup, initialValues,
    backupInfo,
    visible,
    universeList,
    storageConfigs,
  ...
  const restoreBackup = async (values) => {
    if (
  ...
  const hasBackupInfo = () => {
    return isNonEmptyObject(backupInfo);
  };
  ...
  let universeOptions = [];
  const _hasBackupInfo = hasBackupInfo();
  ...
  const _initialValues = {
    ...initialValues,
    storageConfigUUID: _hasBackupInfo
      ? storageOptions.find((element) => {
        return element.value === initialValues.storageConfigUUID;
      })
      : ''
  };
  const isUniverseBackup =
    _hasBackupInfo && Array.isArray(backupInfo.backupList) && backupInfo.backupList.length;
    ...
    <div className="universe-apps-modal" onClick={(e) => e.stopPropagation()}>
      ...
      >
        <Field
          ...
          {...(_hasBackupInfo ? { type: 'hidden' } : null)}
