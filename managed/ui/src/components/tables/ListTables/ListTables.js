...
const TableTitle = (
  {
    ...
    universe,
    numCassandraTables, numRedisTables, numPostgresTables
  }
) => {
  const currentUniverse = universe.currentUniverse.data;
  return (
...
export const ListTables = (props) => {
  const [currentView, setCurrentView] = useState('listTables');
  const { tables } = props;
  const showListTables = () => {
    setCurrentView('listTables');
  };
  ...
  if (isNonEmptyArray(tables.universeTablesList)) {
    tables.universeTablesList.forEach(function (item, idx) {
...
const ListTableGrid = (
  ...
  let listItems = [];
  if (isNonEmptyArray(tables.universeTablesList)) {
    listItems = tables.universeTablesList.map(function (item, idx) {
      return {
