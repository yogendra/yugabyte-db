...
export const CreateBackup = (
  {
    ...
    universeTables,
    visible, isScheduled, tableInfo, storageConfigs,
    initialValues
  }
) => {

  let _backupType = 'ysql';
  if (isNonEmptyObject(tableInfo)) {
    if (tableInfo.tableType === YCQL_TABLE_TYPE) {
      _backupType = 'ycql';
  ...
  const [backupType, setBackupType] = useState(_backupType);
  ...
  const createBackup = async (values) => {
    if (isDefinedNotNull(values.storageConfigUUID)) {
      let _backupType = '';
      if (backupType === 'ysql') {
        _backupType = YSQL_TABLE_TYPE;
      } else if (backupType === 'ycql') {
        _backupType = YCQL_TABLE_TYPE;
      } else if (backupType === 'yedis') {
        _backupType = YEDIS_TABLE_TYPE;
      }
      const payload = {
      ...
      try {
        ...
        } else if (_backupType === YSQL_TABLE_TYPE && isDefinedNotNull(values.tableKeyspace)) {
  ...
  const backupKeyspaceChanged = (props, option) => {
  ...
  const backupTableChanged = (props, option) => {
              ...
              >
                ...
                >
                  ...
                  <Field
                    ...
                    onChange={backupKeyspaceChanged}
